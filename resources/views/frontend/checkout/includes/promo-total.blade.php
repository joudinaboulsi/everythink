	 	...
	 	<td class="checkout_item">
            <div style='margin-right: 20px;'>
            	<a href="{{route('product_details_path', $seo_link)}}" style="display:inline-block;">
            		...
            		<img src="{{$img}}" alt="img" style="width: 100%; height: auto; border:1px solid #c8c6c6; border-radius:5px;">
            	</a>
            </div>
        </td>

        <td style="width:60%">
            <h5 class="no-margin"><a class="grey_text" href="{{route('product_details_path', $seo_link)}}"><b>{{$i->product_name}} @if($i->sku != NULL) - {{$i->sku}}  @endif</a></b></h5>
            <span class="size">{{$i->attr_items}}</span>
        </td>

        <td class="right_align" style="width:20%;">
            <div> 
                <span class="right_align" id="item_price_{{$i->variant_id}}" style="font-size: 16px;"><b>$ {{floatval($i->current_price)}}</b></span>
                @if($i->current_price != $i->regular_price) 
                <span class="small"> <s class='right_align' style="font-weight:normal; font-size:10px; display:block;">$ {{floatval($i->regular_price)}}</s> </span>
                @endif
...
<form id="apply_promo_form">
    ...
    <div class="input-group form-group user-input-wrp">
        <input type="text" class="form-control inputText" placeholder="Gift card or discount code" name="promo_code" autocomplete="off">
    ...
    <span class="badge badge-light promo_valid"> {{session('promo_code')}} <button class="btn btn-danger btn-xs clear_promo">x</button></span>
    @endif
...
<table class="table_width" id="subtotals_table">
    ...
    @endif 
 	...
 	<tr class="tr_30">
 		...
 		<td class="right_align" style="color:#717171; font-size: 0.85714em"><b>
 ...
 <table class="table_width">
 	<tr>
 		<td id="js_total_label" colspan="2" style="width:50%; font-size:16px;">Total<br><span style="font-size: 12px; position: relative; bottom: 7px; color:#717171;" >*all taxes included</span></td>
