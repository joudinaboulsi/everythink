...
{
    public function __construct(CartController $cartController, Cart $cart, UsersApis $usersApis, CheckoutApis $checkoutApis, PromoController $promoController, CartRepository $cartRepository)
    {
    ...
    {
        SEO::setTitle('Everythink | Checkout | Games, Toys and Entertainment online/offline store');
        ...
        SEO::setCanonical(getenv('APP_URL').'checkout');
       ...
       else
      {
        ...
        return view('frontend.checkout.checkout', array('items' =>$cart_data['items'], 'removed_from_cart' => $cart_data['removed_from_cart'], 'addresses' => $addresses, 'countries' => $countries, 'totals' => $totals));
      }
    }
  ...
  {   
   ...
   public function clearPromoCode()
   {
        ...
        SEO::setDescription('Find your gifts in our online store and offline shops in Araya (Main Road) and Saida (The Spot) and send them to your beloved ones all around Lebanon.');
        SEO::opengraph()->setUrl(getenv('APP_URL'));
        SEO::setCanonical(getenv('APP_URL').'clear-promo');
        SEO::opengraph()->addProperty('type', 'articles');
        SEO::twitter()->setSite('@EverythinkMe');
  ...
  { 
        SEO::setTitle('Everythink | Payment | Games, Toys and Entertainment online/offline store');
        SEO::setDescription('Find your gifts in our online store and offline shops in Araya (Main Road) and Saida (The Spot) and send them to your beloved ones all around Lebanon.');
        SEO::opengraph()->setUrl(getenv('APP_URL'));
        SEO::setCanonical(getenv('APP_URL').'payment');
        SEO::opengraph()->addProperty('type', 'articles');
        SEO::twitter()->setSite('@EverythinkMe');
  ...
  {  
      ...
      if($warning_popup['status'] == true) 
      {
        ...
        session(['payment_branch' => $request->input('payment_branch')]);
      ...
      {
          $gift_check = $request->input('gift_check');

          // ====== A ========= Insert order + send email
          $order_id = $this->insertOrder($request, $totals, $cart_data, $gift_check);
  ...
  // ====== A ========= Insert order + send email
  public function insertOrder($request, $totals, $cart_data, $gift_check)
  {
    ...
    if(session('email') !== NULL && session('address') !== NULL  && session('country') !== NULL)  
    {
      ...
      $order_id = $this->checkoutApis->insertOrder($request, $cart_data['items'], $totals['shipping_fees_float'], $totals['promo_type'], $totals['promo_value'], $totals['subtotal_float'], $totals['total_float'], $gift_check);

      // ====== B ========= send email on purchase
      $this->sendPurchaseEmail($cart_data, $totals, $order_id, $gift_check); 
  ...
  // ====== B ========= send email on purchase
  public function sendPurchaseEmail($cart_data, $totals, $order_id, $gift_check)
  {
    ...
    $payment = $this->checkoutApis->getPaymentMethodFromId($order_id);

    // send email to client
    Mail::send('emails.cash-on-delivery-client-email', array('gift_check' => $gift_check, 'cart_data' => $cart_data, 'totals' => $totals, 'order_id' => $order_id, 'purchase' => $purchase, 'shipping_country' => $shipping_country, 'billing_country' => $billing_country, 'payment_method' => $payment[0]->payment_method, 'payment_branch' => $payment[0]->payment_branch), function($message)
    {
    ...
    // send email to admin
    Mail::send('emails.cash-on-delivery-admin-email', array('gift_check' => $gift_check, 'cart_data' => $cart_data, 'totals' => $totals, 'order_id' => $order_id, 'purchase' => $purchase, 'shipping_country' => $shipping_country, 'billing_country' => $billing_country, 'client_email' => session('email'), 'payment_method' => $payment[0]->payment_method, 'payment_branch' => $payment[0]->payment_branch), function($message)
    {
        $message->from('no-reply@'.config('global.COMPANY_DOMAIN'), config('global.COMPANY_NAME'));
        $message->to('info@everythink.me')->subject('Online order');
    });
