...
class ProductsController extends Controller
{
     ...
     * @return void
     */
    public function __construct(PagesController $pagesController, TagsRepository $tagsRepository, CategoriesApis $categoriesApis, ProductApis $productApis, UsersApis $usersApis, TagsApis $tagsApis, SeoRepository $seoRepository)
    {
    ...
    {   
        ...
        $collections = $this->tagsApis->getCollections(8); // get the collections

        if(!empty($collections))
        {
            foreach($collections as $c)
                $c->url_tag_name = PagesController::fixUrlName($c->name);
        }
        ...
        return view('frontend.products.category',  array('category' => $category, 'category_id' => $category_id, 'parentCategories' => $parentCategories, 'brands' => $brands, 
            'show_list' => $show_list, 'interval' => $interval, 'tags' => $tags, 'highlights' => $highlights, 'collections' => $collections, 'now' => Carbon::now('Asia/Beirut')->format('Y-m-d')));
    }
    ...
    {   
        ...
        $product = $this->productApis->show($product_id); //Shows the information of a specific selected product
         $tag =$this->TagsRepository->getProductTags($product_id); 
        // set seo from product table
        ...
        //SEO microtags for FB catalog
        SEO::opengraph()->addProperty('type', 'website');
        SEO::opengraph()->addProperty('product:brand', 'Everythink');
        /*SEO::opengraph()->addProperty('product:availability', 'in stock');*/
        SEO::opengraph()->addProperty('product:condition', 'new');
        SEO::opengraph()->addProperty('product:price:amount', $actual_price);
        SEO::opengraph()->addProperty('product:price:currency', 'USD');
        SEO::opengraph()->addProperty('product:price:retailer_item_id', $product[0]->product_id);

        return view('frontend.products.product', array('product' => $product,
        'tag'=>$tag, 'images' => $images, 'attributes' => $attributes, 'attrItems' => $attrItems , 'variants' => $variants, 'similarProducts' => $similarProducts, 'linkedProducts' => $linkedProducts, 'prices' => $prices, 'actual_price' => $actual_price, 'old_price' => $old_price));
    }
        ...
        <div class="product">
            ...
            <div class="image">
                ...
                <a href="<?php echo route('product_details_path', $seo_link); ?>">
                    <img src="<?php echo getenv('S3_URL').'/products/'.$product->img; ?>" alt="Product Image" class="img-responsive">
                </a>
...
$tags = $this->TagsRepository->getProductTags( $product->product_id);
     
        return view('frontend.products.includes.product-list', array('categoryProductsNb' => 100, 'show_list' => $showFilteredProductByCategory, 'tags'=>$tags,'interval' => $interval, 'now' => Carbon::now('Asia/Beirut')->format('Y-m-d')));
    }
