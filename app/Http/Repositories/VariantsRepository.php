...
class VariantsRepository {
    ...
    {
        $q = \DB::select("SELECT A.variant_id, A.product_id, A.img, A.sku, A.regular_price, A.sales_price, A.sales_price_start_date, A.sales_price_end_date, A.stock_qty, A.stock_status_id, A.is_main, A.hidden, GROUP_CONCAT(C.name SEPARATOR ', ') as item_name, COUNT(C.attribute_item_id) as items_nb, group_concat(D.img) as item_img
                          FROM variants as A
