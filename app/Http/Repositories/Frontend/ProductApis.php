...
class ProductApis {
    ...
    {
        $q = \DB::select("SELECT A.*, B.name as brand, C.img as primary_img, D.*
                          FROM products as A
    ...
    public function showProductListByCategory($category_id)
    {
        $q = \DB::select("SELECT A.*, B.*
                          ...
                          array(':category_id' => $category_id, ':category_id2' => $category_id));
       return $q;
    }
    ...
    {
        $q = \DB::select("SELECT MIN(A.regular_price) as min_regular_price, MAX(A.regular_price) as max_regular_price, MIN(A.current_price) as min_current_price, MAX(A.current_price) as max_current_price
                          FROM
    ...
    {
        $q = \DB::select("SELECT A.variant_id, A.product_id, A.regular_price, A.img, A.sales_price, A.sales_price_start_date, A.sales_price_end_date, A.stock_qty, A.stock_status_id, A.is_main, GROUP_CONCAT(C.attribute_item_id SEPARATOR '_') as item_code
                          FROM variants as A
    ...
    ------------------------------------*/
    public function getAttributeItemsWeCanClick($product_id, $attribute_items_selected, $nb_attribute_items)
    {
         //Removing the "" from code.
         $attribute_items_selected = str_replace('"', "", $attribute_items_selected);
         $q = \DB::select("SELECT *
                          ...
                          WHERE A.variant_id IN (SELECT  A.variant_id
                                                  ...
                                                  WHERE A.attribute_item_id IN (".$attribute_items_selected.")
                                                  GROUP BY A.variant_id
                                                  HAVING COUNT(*) = :nb_attribute_items)
                          AND A.attribute_item_id NOT IN (".$attribute_items_selected.")",
                          array(':product_id' => $product_id, 'nb_attribute_items' => $nb_attribute_items)
                          );
        return $q;
    }
    ...
    ------------------------------------*/
    public function getVariantPriceIntervalByAttributes($product_id, $attribute_items_selected, $nb_attribute_items)
    {
      //Removing the "" from code.
      $attribute_items_selected = str_replace('"', "", $attribute_items_selected);
      //$attribute_items_selected = intval($attribute_items_selected);
       $q = \DB::select("SELECT MIN(A.regular_price) as min_regular_price, MAX(A.regular_price) as max_regular_price, MAX(current_price) as max_current_price, MIN(current_price) as min_current_price
                          FROM
    ...
    Shows the list of variants based on attribute selection
    ------------------------------------*/
    public function getVariantsByAttributes($product_id, $attribute_items_selected, $nb_attribute_items)
    {

      //Removing the "" from code.
      $attribute_items_selected = str_replace('"', "", $attribute_items_selected);
      //$attribute_items_selected = intval($attribute_items_selected);
       $q = \DB::select("SELECT B.variant_id, B.regular_price, B.sales_price, IF(B.sales_price_end_date >= NOW() AND B.sales_price_start_date <= NOW(), B.sales_price, B.regular_price) as current_price,
   ...
   /*----------------------------------
    ...
    {
        $q = \DB::select("SELECT A.attribute_id, A.name, A.is_main, group_concat(B.name) as attribute_names
    ...
    {
        $q = \DB::select("SELECT A.attribute_id, A.product_id, A.name as attribute_name, B.name as item_name, B.attribute_item_id, B.img as attribute_item_img
                          FROM attributes as A
    ...
    public function getSimilarProducts($product_id)
    {
        $q = \DB::select("SELECT A.*, B.*
                          ...
                          ORDER BY RAND()
                          LIMIT 10",
                          array(':product_id' => $product_id, ':product_id2' => $product_id, ':product_id3' => $product_id));
        return $q;
    }
    ...
    public function getLinkedProducts($product_id)
    {
        $q = \DB::select("SELECT A.*, B.*
                          ...
                          ORDER BY RAND()
                          LIMIT 10",
                          array(':product_id' => $product_id, ':product_id2' => $product_id));
    ...
    {
        $q = \DB::select("SELECT A.*, B.*, C.*, D.name as brand, E.img
  ...
  /*----------------------------------
    ...
    public function showFilteredProductByCategory($category_id)
    {
        ...
        //Building the query
        $query = $query."
                 SELECT A.product_id, A.min_price,  A.total_stock_qty,  A.total_stock_status, B.*
                  FROM
                  (#get the minimum price and the total stock qty and status to make sure that we have at least 1 variant with stocks
                  SELECT A.product_id, MIN(A.current_price) as min_price, SUM(A.stock_qty) as total_stock_qty, SUM(A.stock_status_id) as total_stock_status
                  FROM
        ...
        $bind =  array(':category_id' => $category_id, ':category_id2' => $category_id);

        //If isset Min and Max Price session
        if(isset($min_price) and isset($max_price))
        {
           //Building the query
           $query = $query."AND A.min_price BETWEEN :min_price AND :max_price ";
        ...
        {
           //Building the query
           $query = $query." AND B.sales_price_start_date <= :current_date_3 AND B.sales_price_end_date >= :current_date_4 ";
    ...
    {
        $q = \DB::select("SELECT D.brand_id, D.name
                          FROM products_has_categories as A
                          JOIN products as B ON A.product_id = B.product_id AND B.hidden=0
    ...
    {
        $q = \DB::select("SELECT A.tag_id, B.name
    ...
    {
        $q = \DB::select("SELECT A.*, B.name, B.enable_stock_mgmt
                          FROM variants as A
                          JOIN products as B ON A.product_id = B.product_id AND B.hidden=0
    ...
    public function getDeals($limit)
    {
      $query ='SELECT A.*, B.*
                         ...
                         D.img as img, A.created_at
                          ...
                          else // if we don't want to limit the number of product to be display, $limit = false
                          {
    ...
    /*----------------------------------
    Show new added products
    ------------------------------------*/
    public function getNew()
    {

        $q = \DB::select("SELECT A.*, B.*, C.name as brand
                          FROM
                          (#get the minimum price and the total stock qty and status to make sure that we have at least 1 variant with stocks
                          SELECT A.product_id, A.brand_id, MIN(A.current_price) as min_price, SUM(A.stock_qty) as total_stock_qty, SUM(A.stock_status_id) as total_stock_status
                          FROM
                          (
                          SELECT A.product_id, A.brand_id, IF(B.sales_price_end_date >= NOW() AND B.sales_price_start_date <= NOW(), B.sales_price, B.regular_price) as current_price,
                          B.stock_qty, B.stock_status_id
                          FROM products as A
                          JOIN variants as B ON A.product_id = B.product_id AND B.hidden = 0
                          WHERE A.hidden=0
                          ...
                          B.stock_qty as stock_qty, B.stock_status_id as stock_status_id,
                          IF(B.sales_price_end_date >= NOW() AND B.sales_price_start_date <= NOW(), B.sales_price, B.regular_price) as current_price,
                          D.img as img, MAX(E.img) as img_hover, A.created_at
                          FROM products as A
                          JOIN variants as B ON A.product_id = B.product_id AND B.hidden = 0
                          LEFT JOIN product_images as D ON A.product_id = D.product_id AND D.is_primary = 1
                          LEFT JOIN product_images as E ON A.product_id = E.product_id AND E.is_primary = 0 AND E.hidden = 0
                          WHERE A.hidden=0
                          GROUP BY A.product_id, current_price
                          ) as A
                          ) as B ON A.product_id = B.product_id AND A.min_price = B.current_price
    ...
    public function getProductsOfCollection($tag_id)
    {
        $q = \DB::select("SELECT A.*, B.*
                        ...
                        array(':tag_id' => $tag_id, ':tag_id_2' => $tag_id));
       return $q;
    }
    ...
    public function search($keyword)
    {
      ...
      $q = \DB::select("SELECT A.product_id, A.name, B.img
                        FROM products as A
