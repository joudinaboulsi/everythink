...
class AttributesRepository {
    ...
    {
        $q = \DB::select("SELECT A.attribute_id, A.product_id, A.name as attribute_name, B.name as item_name, B.attribute_item_id
                          FROM attributes as A
    ...
    {
        $q = \DB::select("SELECT A.attribute_item_id, A.name as item_name, A.img, B.name as attribute_name, B.attribute_id
                          FROM attribute_items as A
                          JOIN attributes as B ON B.product_id=:product_id AND A.attribute_id=B.attribute_id AND B.is_main=1",
    ...
    public function addMainAttributeItem($request,$img)
    {
        try
        {
          \DB::transaction(function () use ($request,$img){
            ...
            // Returns attribute id after insterting the main attribute in attributes table
