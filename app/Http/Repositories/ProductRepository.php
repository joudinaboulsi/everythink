...
class ProductRepository {
    ...
    public function updateBasicProduct($request)
    {
      ...
      if($request->input('publish_unpublish') == '1')
      {
        ...
        $main_variant = \DB::select("SELECT A.variant_id 
                              FROM variants as A
        ...
        \DB::transaction(function () use ($request, $main_variant, $stock_management)
        {
         
          // Get the attributes of a specific product
          $product_attributes = \DB::select("SELECT A.product_id FROM attributes as A WHERE A.product_id = :product_id",
                      array(':product_id' => $request->input('edit_product_id')));
          ...
          $variant_attribute_items = \DB::select("SELECT attribute_item_id
                        ...
                        array(':variant_id' => $main_variant[0]->variant_id));

          $msg = [];
          if(count($product_attributes) == count($variant_attribute_items))
          {
             ...
             ->update(
                  array('name' => $request->input('edit_product_name'),
              ...
              $msg = ['status' => 'success', 'message' => 'Product successfully saved and published'];
              $request->session()->put('msg', $msg);
          }
          else
          {   
              $msg = ['status' => 'error', 'message' => 'Cannot Publish Product Because Number Of Attributes Linked to Product is Incorrect'];
              $request->session()->put('msg', $msg);
          }
        });
      ...
      else
      {
          \DB::table('products')
           ...
           return $msg = ['status' => 'error', 'message' => 'Product is unpublished'];
      }
    }
      ...
      try
        {
          \DB::transaction(function () use ($request, $file_name){
              ...
              $variant_list = \DB::select("SELECT variant_id 
                                FROM variants
                   ...
                   foreach($request->input('categories') as $c)
                       {
                         ...
                         array_push($categories, array('product_id' => $product_id,
                                                       'category_id' => $c,
                                                       'created_by' => Auth::id(),
                                                       'updated_by' => Auth::id(),
                                                       'created_at' => Carbon::now('Asia/Beirut'),
    ...
    {
      // Check if the product is published or not
      $product = \DB::select("SELECT hidden 
                                FROM products
                                WHERE product_id = :product_id",
                        array(':product_id' => $request->input('id')));

      if($product[0]->hidden == 1) //product is unpublished
      {
        $hidden = 0;
        // Check if the product is published or not
        $main_variant = \DB::select("SELECT A.variant_id 
        ...
        \DB::transaction(function () use ($request, $main_variant, $hidden)
        {
          //Update variant publishing status
          \DB::table('variants')
             ->where('variant_id', '=', $main_variant[0]->variant_id)
    ...
    public function feature($request)
    {
      ...
      $product = \DB::select("SELECT featured 
                                FROM products
                                WHERE product_id = :product_id",
                        array(':product_id' => $request->input('id')));

      if($product[0]->featured == 1) //product is featured
        $featured = 0;
      else
        $featured = 1;
      ...
      try
        {
          \DB::transaction(function () use ($request){
            ...
            \DB::select("DELETE FROM variants_has_attribute_items
                          WHERE variant_id IN
                          (
                            SELECT variant_id
                            FROM variants  
            ...
            \DB::select("DELETE FROM attribute_items
    ...
    {
        $q = \DB::select("SELECT A.product_id, A.name, B.img, A.brand_id, A.description, A.short_description, A.enable_stock_mgmt, A.allow_backorders, A.purchase_note, A.order_position, A.seo_title, A.seo_slug, A.seo_meta_desc, A.seo_meta_keywords, A.hidden,
                                 B.img, 
    ...
    { 
        $q = \DB::select("SELECT MIN(A.regular_price) as min_regular_price, MAX(A.regular_price) as max_regular_price, MIN(A.sales_price) as min_sales_price, MIN(A.current_price) as min_current_price, MAX(A.current_price) as max_current_price
                          FROM
    ...
    {
        $q = \DB::select("SELECT product_id, product_img_id, img, is_primary
                          FROM product_images
                          WHERE product_id=:product_id
                          AND hidden=0",
            array(':product_id' => $product_id));
    ...
    public function updatePrimaryImage($request, $img)
    {
        \DB::table('product_images')
             ->where('product_img_id', '=', $request->input('edit_primary_img_id'))
             ->update(
                  array('img' => $img)
             );
    }
    ...
    public function deleteImage($request)
    {
        ...
        array(':product_img_id' => $request->input('id')
              )
        ); 
    ...
    public function updateDescription($request)
    {
      ...
      \DB::table('products')
           ->where('product_id', '=', $request->input('edit_product_id'))
           ->update(
                array('short_description' =>$request->input('edit_short_description'),
    ...
    public function updateSeo($request)
    {
      ...
      \DB::table('products')
           ->where('product_id', '=', $request->input('edit_product_id'))
           ->update(
                array('seo_title' => $request->input('seo_title_edit'),
                       'seo_slug' => $request->input('seo_slug_edit'),
                       'seo_meta_desc' => $request->input('seo_meta_desc_edit'),
                       'seo_meta_keywords' => $request->input('seo_meta_keywords_edit'),
                       'updated_by' => Auth::id(),
                       'updated_at' => Carbon::now('Asia/Beirut')
                     )
           ); 
        ...
        try
        {
          \DB::transaction(function () use ($request){
            ...
            \DB::table('products_has_videos')->insert(
                     array('product_id' => $request->input('video_product_id'),
       ...
       try
        {
          \DB::transaction(function () use ($request){
            ...
            \DB::select("DELETE 
                ...
                array(':video_id' => $request->input('id')
                      )
                );
          });
    ...
    {
        $p = \DB::select("SELECT A.*, B.cat_name, A.brand, A.enable_stock_mgmt, C.min_stock_qty, C.stock_status_id
                          FROM ( 
                            SELECT A.product_id, A.prod_name, A.primary_img as primary_img, A.sales_price as sales_price, MIN(A.current_price) as min_price, MAX(A.current_price) as max_price, A.hidden, A.enable_stock_mgmt, A.brand
                            FROM
                            (
                       ...
                       ",
                        array(':product_id' => $request->input('id'), ':product_id2' => $request->input('id')));
        return $p;
    }
    ...
    public function addLinkedProduct($request)
    {
        try
        {
            ...
            {
              // build the component table
              array_push($product_list, array('current_product_id' => $request->input('link_product_id'),
                                'linked_product_id' => $p,
                                'created_by' => Auth::id(),
                                'updated_by' => Auth::id(),
                                'created_at' => Carbon::now('Asia/Beirut'),
       ...
       try
        {
          \DB::transaction(function () use ($request){
            ...
            \DB::select("DELETE 
                ...
                array(':linked_product_id' => $request->input('linked_product_id'),
    ...
    public function getLinkedProducts($product_id)
    {
        ...
        $p = \DB::select("SELECT B.product_id, B.name, C.img
